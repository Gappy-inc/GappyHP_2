...
export default function LanguageSwitcher({ mobile = false }: { mobile?: boolean }) {
  ...
  return (
    <nav aria-label={current === 'ja' ? '言語を選択' : 'Select language'} className={`flex items-center font-mono text-[10px] tracking-[0.12em] ${mobile ? 'justify-center border border-white/25 p-1' : ''}`}>
