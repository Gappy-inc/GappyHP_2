...
export function AxisMark({ inverse = false, className = '' }: { inverse?: boolean; className?: string }) {
  return (
    <span className={`relative block h-8 w-8 ${className}`} aria-hidden="true">
      ...
      <span className="axis-signal absolute left-[calc(50%_-_2px)] top-[4px] z-10 h-1 w-1 rounded-full bg-gold-500" />
...
export function GappyAxis({ inverse = false, label = 'Gappy Axis' }: { inverse?: boolean; label?: string }) {
  return (
    <div className={`relative min-h-[460px] overflow-hidden border ${inverse ? 'border-white/15 bg-navy-950' : 'border-navy-900/15 bg-ivory-100'}`}>
      ...
      <span className="absolute left-[calc(50%_-_18px)] top-[17%] h-0 w-0 border-b-[22px] border-l-[18px] border-r-[18px] border-b-gold-500 border-l-transparent border-r-transparent" aria-hidden="true" />
    ...
    <ol className={`border-t ${inverse ? 'border-white/20' : 'border-navy-900/20'}`}>
      {items.map((item, index) => (
        <li key={item} className={`capability-row grid min-h-20 grid-cols-[3rem_1fr_auto] items-center border-b ${inverse ? 'border-white/20 text-white' : 'border-navy-900/20 text-navy-900'}`}>
          <span className={`font-mono text-[12px] ${inverse ? 'text-gold-300' : 'text-gold-700'}`}>{String(index + 1).padStart(2, '0')}</span>
          <span className="text-base font-medium md:text-lg">{item}</span>
          <span className="capability-arrow text-gold-500" aria-hidden="true">→</span>
        </li>
      ))}
    </ol>
  )
